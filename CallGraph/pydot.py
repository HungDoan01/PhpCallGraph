...
import copy
try:
    import CallGraph.dot_parser as dot_parser
except Exception:
...
class Graph(Common):
    ...
    def del_edge(self, src_or_list, dst=None, index=None):
        ...
        if isinstance( src_or_list, (list, tuple)):
            if dst is not None and isinstance(dst, (int, int32)):
                index = dst
            src, dst = src_or_list
        else:
